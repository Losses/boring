...
class Main {
 ...
 static function opt(o:JsonValue,n:String):Null<String> { var v=get(o,n); return switch(v) { case JNull: null; case JString(x): x; case JObject(_): fail("invalid "+n); null; case JArray(_): fail("invalid "+n); null; case JNumber(_): fail("invalid "+n); null; case JBool(_): fail("invalid "+n); null; }; }
  static function readMetadata(path:String,pp:String):JsonValue {
  var result:JsonValue=JNull;
  try { result=Json.read(Fs.readFileSync(path,"utf8")); }
 ...
 static function writePub(root:String,a:Array<Release>):Void {var ns=names(a,"pub");for(ni in 0...ns.length){var n=ns[ni],g=group(a,"pub",n),vs:Array<JsonValue>=[];for(i in 0...g.length){var r=g[i],u:String=r.url==null?"":r.url;vs.push(O([{name:"version",value:S(r.version)},{name:"archive_url",value:S(u)},{name:"archive_sha256",value:S(r.digest)},{name:"pubspec",value:r.pubspec==null?JNull:r.pubspec}]));}var rev:Array<JsonValue>=[];var vi2=g.length-1;while(vi2>=0){rev.push(vs[vi2]);vi2=vi2-1;}put(root,"pub/api/packages/"+n,Json.write(O([{name:"name",value:S(n)},{name:"latest",value:S(latest(g))},{name:"versions",value:A(rev)}])));}}
 ...
 static function writeSwift(root:String,scope:String,a:Array<Release>):Void {
  ...
  for(ni in 0...ns.length) { var n=ns[ni],g=group(a,"swift",n),rs:Array<JsonField>=[];
   ...
   for(i in 0...g.length) { var r=g[i],ps:String=r.packageSwift==null?"":r.packageSwift; put(root,"swift/"+scope+"/"+n+"/"+r.version+".json",Json.write(O([{name:"id",value:S(scope+"."+n)},{name:"version",value:S(r.version)},{name:"resources",value:A([O([{name:"name",value:S("source-archive")},{name:"type",value:S("application/zip")},{name:"checksum",value:S(r.digest)}])])},{name:"metadata",value:O([])}]))); put(root,"swift/"+scope+"/"+n+"/"+r.version+"/Package.swift",ps); }
  }
 }
 ...
 public static function main():Void {try{var av=NodeProcess.argv;if(av.length==3&&(av[2]=="--help"||av[2]=="-h")){Console.error(USAGE);NodeProcess.exit(0);}var t:Null<String>=null,o:Null<String>=null,b:Null<String>=null,s:Null<String>=null,ar:Null<String>=null,i=2;while(i<av.length){if(i+1>=av.length)fail(USAGE);var flag=av[i];if(flag=="--tree")t=av[i+1];else if(flag=="--output")o=av[i+1];else if(flag=="--base-url")b=av[i+1];else if(flag=="--swift-scope")s=av[i+1];else if(flag=="--archive-base")ar=av[i+1];else fail("unknown flag "+flag);i+=2;}if(t==null||o==null||b==null)fail("required flags missing\n"+USAGE);var tree:String=t==null?"":t,output:String=o==null?"":o,baseUrl:String=b==null?"":b;if(!urlOk(baseUrl))fail("invalid base URL (http or https required)");if(Fs.existsSync(output)&&Fs.readdirSync(output).length>0)fail("output directory is not empty");var all=scan(tree);if(names(all,"swift").length>0&&(s==null||s==""))fail("Swift requires --swift-scope");if(names(all,"swift").length>0&&(ar==null||ar==""))fail("Swift requires --archive-base");if(ar!=null&&!urlOk(ar))fail("invalid archive-base URL (http or https required)");var swiftScope:String=s==null?"":s,archiveBase:String=ar==null?"":ar;validate(all,swiftScope);mkdir(output);writeNpm(output,all);writePub(output,all);writeCargo(output,baseUrl,all);if(names(all,"swift").length>0)writeSwift(output,swiftScope,all);writeMaven(output,all);put(output,"swift/identifiers",Json.write(A([])));put(output,"_headers",headers());put(output,"_redirects",redirects(swiftScope,archiveBase,all));}catch(e:RegistryException){Console.error(e.message);NodeProcess.exit(1);}}
}
