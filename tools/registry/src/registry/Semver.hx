...
class SemverException extends haxe.Exception {
 ...
 static function describe(f:SemverFault):String { var out=""; switch(f) { case InvalidVersion(label, version): out=label+": version "+version+" is outside semver"; } return out; }
}
class Semver {
 ...
 public static function parse(version:String):Null<Version> {
  ...
  var pre:Array<String>=[];if(dash>=0){core=version.substr(0,dash);var end=plus>=0?plus:version.length;if(!validIdentifiers(version.substr(dash+1,end-dash-1)))return null;pre=version.substr(dash+1,end-dash-1).split(".");}
  var parts=core.split(".");if(parts.length!=3||!validNumeric(parts[0])||!validNumeric(parts[1])||!validNumeric(parts[2]))return null;
  return {major:Std.parseInt(parts[0]),minor:Std.parseInt(parts[1]),patch:Std.parseInt(parts[2]),pre:pre};
 }
 ...
 static function pre(a:Array<String>, b:Array<String>):Int {
  ...
  for(i in 0...n) { final x=a[i], y=b[i], xd=validNumeric(x), yd=validNumeric(y); if(xd&&yd) { final d=Std.parseInt(x)-Std.parseInt(y); if(d!=0)return d; } else if(xd!=yd) return xd ? -1 : 1; else if(x!=y) return x<y ? -1 : 1; }
