...
@:keep
class Semver {
 static function intOf(x:String):Int { var out=0; for(i in 0...x.length) out=out*10+StringTools.codeAt(x,i)-48; return out; }
 /** Copies a split into a fresh Array so the List result of split never
 flows into a variable consumers treat as MutableList. */
 static function splitParts(s:String):Array<String> { var out:Array<String>=[]; var ps=StringTools.split(s,"."); for(i in 0...ps.length) out.push(ps[i]); return out; }
 ...
 static function validNumeric(x:String):Bool { if(x.length==0)return false;if(x.length>1&&StringTools.codeAt(x, 0)==48)return false;for(i in 0...x.length){var c=StringTools.codeAt(x, i);if(c<48||c>57)return false;}return true; }
 static function validIdentifier(x:String):Bool { if(x.length==0)return false;for(i in 0...x.length){var c=StringTools.codeAt(x, i);if(!((c>=48&&c<=57)||(c>=65&&c<=90)||(c>=97&&c<=122)||c==45))return false;}return true; }
 static function validIdentifiers(x:String):Bool {var a=StringTools.split(x,".");for(i in 0...a.length)if(!validIdentifier(a[i]))return false;return true;}
 ...
 static function parseFault(version:String):Null<SemverFault> {
  var core=version, dash=registry.StringTools.indexOf(version,"-"), plus=registry.StringTools.indexOf(version,"+");
  ...
  var parts=StringTools.split(core,"."); if(parts.length!=3||!validNumeric(parts[0])||!validNumeric(parts[1])||!validNumeric(parts[2])) return InvalidCore(version);
  return null;
 }
 ...
 static function parseUnvalidated(version:String):Version {
  var dash=registry.StringTools.indexOf(version,"-"), plus=registry.StringTools.indexOf(version,"+");
  ...
  var parts=StringTools.split(core,".");
 ...
 static function pre(a:Array<String>, b:Array<String>):Int {
  ...
  for(i in 0...n) { final x=a[i], y=b[i], xd=validNumeric(x), yd=validNumeric(y); if(xd&&yd) { final d=intOf(x)-intOf(y); if(d!=0)return d; } else if(xd!=yd) return xd ? -1 : 1; else if(x!=y) return StringTools.compare(x,y); }
