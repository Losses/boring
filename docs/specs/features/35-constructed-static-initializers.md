...
| Argument grammar: any expression the emitters render | Compile-time delegation to five renderers. | Acceptance depends on five renderer implementations agreeing; a partial renderer agreement surfaces as a per-target generation failure instead of a named error. | No classifier to maintain. | No enumerated language to read; the error for a rejected form is the renderer's own. |
