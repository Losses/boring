...
This specification rules the translation of Haxe classes, member variables, properties, interfaces, and access modifiers (`public`, `private`, `static`, `inline`) into Rust, TypeScript, and Kotlin. In the current codebase, classes appear in Haxe in `haxe/src/boring/BinaryReader.hx` (lines 11-57), `haxe/src/boring/BinaryWriter.hx` (lines 11-47), and `haxe/src/boring/VectorCodec.hx` (lines 10-54), extern classes appear in `haxe/src/boring/Process.hx` (lines 8-10) and `haxe/src/boring/Console.hx` (lines 8-10), structs and `impl` blocks appear in Rust in `rust/src/lib.rs` (lines 9-22, 51-82), and classes and interfaces appear in TypeScript in `ts/src/codec.ts` (lines 10-117) and `ts/src/records.ts` (lines 7-18). No Kotlin implementation exists yet; the Kotlin rulings bind generated code.
