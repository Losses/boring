...
This specification rules the translation of `haxe.Exception`, exception propagation, and catch block mechanics into Rust and TypeScript. In the current codebase, exceptions appear in Haxe in `haxe/src/boring/VectorCodec.hx` (lines 32, 50) and `tests/haxe/Main.hx` (lines 97-103), structured error variants appear in Rust in `rust/src/lib.rs` (lines 28-48, 62, 66, 79, 85, 102, 106, 128) and `tests/rust/vector.rs` (lines 72-96), and thrown errors appear in TypeScript in `ts/src/vector-format.ts` (lines 34, 49) and `tests/ts/codec.test.ts` (lines 66-80).
...
`haxe.Exception` translates to `Result<T, VectorError>` return types in Rust with explicit enum variants, and to standard `Error` instances thrown with descriptive message strings in TypeScript, adhering to `docs/specs/features/06-errors-and-results.md`.

The exact mapping of exception types and messages to Rust variants is:
- `'bad vector magic'` maps to `VectorError::BadMagic`.
- `'record count exceeds u32'` maps to `VectorError::CountOverflow`.
- `'vector ended mid-record'` maps to `VectorError::UnexpectedEof`.
- `'trailing bytes in vector: ${remaining}'` maps to `VectorError::TrailingBytes { remaining }`.
