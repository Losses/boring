...
| 22 | [22-default-argument-expansion.md](features/22-default-argument-expansion.md) | Complete | Optional function parameters with default values: the completion pass in the typed common layer, the call sites that omit them, Stages A and B implemented; Stage C implemented: normalization bindings and constructor defaults. |
...
| 26 | [26-package-registry.md](features/26-package-registry.md) | Planned | The publish manager: it scans the GitHub releases of a repository list and generates one static registry site, with artifacts served from release assets and object storage and only documents on the site. The tool itself is Haxe in the translatable subset, compiled through the TypeScript target. |
...
| 16 | [16-dataclass-sorted-keys.md](stdlib/16-dataclass-sorted-keys.md) | Complete | Lexicographic ordering and comparator eligibility for `@:dataClass` keys in sorted collections. |
| 17 | [17-platform-modules.md](stdlib/17-platform-modules.md) | Complete | Host-backed filesystem, environment, process-argument, and path module routing on each target. |
