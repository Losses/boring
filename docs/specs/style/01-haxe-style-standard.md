...
| `V18 NonAsciiStringIndex` | A string index operation on non-ASCII content, or a `String.fromCharCode` argument outside the byte domain, per `docs/specs/features/08-strings-and-unicode.md` | Untyped call access (`charCodeAt`, `charAt`, `codePointAt`, `substring`, `substr`, `indexOf`, `lastIndexOf`) whose subject is a string literal above U+007F or a final local initialized from one; typed `length` and surviving call forms on those subjects plus fields initialized from a string literal that receive no assignment; a `String.fromCharCode` integer literal outside 0..255 |
