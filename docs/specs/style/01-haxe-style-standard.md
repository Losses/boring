...
This specification defines the language style standard for the Haxe source accepted by the translation pipeline, and the interception that enforces it. The interception runs before generation on every pipeline entry, rejects non-conforming code and unsupported features with a named violation at the offending site, and aborts the run. Haxe source inside the pipeline keeps the friendly source syntax of the language; every restriction below exists because the corresponding construct either has no translation with identical observable behavior on Rust, TypeScript, and Kotlin, or its translated form has uncontrollable cost on the JavaScript target.
...
| --- | --- | --- |
| `V01 IteratorLoop` | `for (item in subject)` where the subject's static type is neither an integer range nor `Array<T>`/`ReadOnlyArray<T>` (features/09 amendment of 2026-09-01) | typed `TFor` whose iteration subject's static type is neither an integer range nor `Array<T>`/`ReadOnlyArray<T>` |
| `V02 FunctionalIteration` | `Lambda` module calls, array methods taking function values (`map`, `filter`, `fold`, and the rest listed in `docs/specs/features/09-iterators.md`), and comparator `sort`; sorting goes through the named strategies of `docs/specs/features/17-sorting.md`. Exempt: the closed-list calls of `docs/specs/macros/01-functional-idiom-expansion.md` whose argument is syntactically an inline function literal; those expand into loop forms before emission, and a closed-list call in any other shape moves to the macro spec named errors | untyped `ECall` whose callee field is one of the listed method names, or `sort` with an argument, minus the macros/01 exemption |
...
| `V11 Int64Misuse` | An `haxe.Int64` operation or crossing outside the supported constructors, fixed-width arithmetic, bitwise operations, shifts, high/low extraction, and named boundary conversions of standard library spec 05 | Typed expression whose Int64 operation or conversion is absent from the capability table in `docs/specs/stdlib/05-haxe-int64.md` |
...
| `V16 NonConstantDefault` | A default outside the two sanctioned classes of `docs/specs/features/22-default-argument-expansion.md`: in default position, after compile-time constant evaluation, anything beyond literals, `null`, and argument-less enum constructors; and a coalescing site whose expression falls outside that specification's class, or whose parameter is consumed more than once | Typed declaration of a defaulted parameter or a coalescing site outside the sanctioned classes |
...
| `V18 NonAsciiStringIndex` | A string index operation on non-ASCII content, a `StringTools` character call, or a `String.fromCharCode` argument outside the byte domain, per `docs/specs/features/08-strings-and-unicode.md`; non-ASCII content uses `std.UString` (`docs/specs/stdlib/10-unicode-string-access.md`) | Untyped call access (`charCodeAt`, `charAt`, `codePointAt`, `substring`, `substr`, `indexOf`, `lastIndexOf`) whose subject is a string literal above U+007F or a final local initialized from one; any `StringTools.fastCodeAt` or `StringTools.fromCharCode` call; typed `length` and surviving call forms on those subjects plus fields initialized from a string literal that receive no assignment; a `String.fromCharCode` integer literal outside 0..255 |
| `V19 TryRegionControlFlow` | A `return`, `break`, or `continue` inside a try-region body, per the catch-site contract of `docs/specs/features/06-errors-and-results.md`; `return` is legal in a handler, where control leaves the region through the handler edge | Typed `TReturn` always, typed `TBreak`/`TContinue` outside an enclosing loop, within a `TTry` body whose catch list has exactly one clause; nested region bodies reset the loop context, and function values inside the body bind their own return |
| `V20 TryRegionMixedDomains` | A try region catching more than one domain, or a region body throwing a payload class other than the one its clause catches, per the one-domain-per-region rule of `docs/specs/features/06-errors-and-results.md` | Typed `TTry` whose catch list length differs from one; a catch clause whose class carries no payload enum; a `TThrow` inside a region body whose thrown class module is neither the caught module nor a domain an enclosing region absorbs |
...
2. Pass 1 runs as a `@:build` macro over the untyped field expressions of every class, injected with `Compiler.addGlobalMetadata` and guarded by the source paths. The typer expands `array.map(fn)` into an inlined loop holding the function value and inlines `Reflect.hasField` into a prototype call; the untyped tree is the only layer where those source constructs survive, so `V02` through `V03` and the `V05` source forms check there. `V01` needs the iteration subject's static type and checks in pass 2, per the features/09 amendment of 2026-09-01.
