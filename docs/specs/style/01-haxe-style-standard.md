...
| `V02 FunctionalIteration` | `Lambda` module calls, array methods taking function values (`map`, `filter`, `fold`, and the rest listed in `docs/specs/features/09-iterators.md`), and comparator `sort`; sorting goes through the named strategies of `docs/specs/features/17-sorting.md`. Exempt: the closed-list calls of `docs/specs/features/21-functional-idiom-expansion.md` whose argument is syntactically an inline function literal; those expand into loop forms before emission, and a closed-list call in any other shape moves to the features/21 named errors | untyped `ECall` whose callee field is one of the listed method names, or `sort` with an argument, minus the features/21 exemption |
