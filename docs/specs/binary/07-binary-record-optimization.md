...
The rewrite runs in the typed common layer, the same layer as the completion pass of `features/22-default-argument-expansion.md`, before any target compiler runs. It changes three things in the program: parameter lists, call-site argument lists, and, where a runtime condition chooses between buffers of one kind, the merged storage's type and its pair constructions (rulings 10 and 12).
...
12. **Pair lowering.** The pair of ruling 10 is a compile-time value: its object form exists in the emitted code only where a heap slot is the value's storage. The rewrite classifies every merged storage location, using the same whole-program facts as the stream tracking, and lowers it per class. **Local**: a merged value whose uses stay inside one function body after inlining is two scalars, the buffer reference and the position integer, and the emitted code constructs no pair. **Call boundary**: a merged value crossing a non-inlined internal call in either direction passes as a value struct on Rust and Swift, costing no allocation, and through a caller-owned holder on TypeScript, Kotlin, and Dart, a holder the callee writes, one per concurrently live value at the call site, hoisted out of any enclosing loop, so a call in steady state allocates nothing. **Object field**: a merged value held by a declaring object lowers to two fields of that object, `buffer` and `pos`, on TypeScript, Kotlin, and Dart, and to one struct-valued field on Rust and Swift; no member object is constructed on any target. **Array element**: a merged value stored element-wise lowers to two parallel arrays, one of buffers and one of positions, grown and indexed together, on TypeScript, Kotlin, and Dart, and to one array of structs on Rust and Swift; an element store allocates nothing on any target. **Other structures**: a merged value stored in any other structure, a sorted table entry for example, allocates the pair object, one object per stored value; this is the residual class. **Foreign boundary**: no merged value crosses the foreign-facing boundary, because the deep check of binary spec 08 places pair-typed declarations in the system tier and the boundary conversion of that specification runs first. Reading the two members has identical semantics in every class; the classes differ only in emitted shape.
