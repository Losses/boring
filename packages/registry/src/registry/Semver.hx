...
class Semver {
    ...
    static function validNumeric(x:String):Bool {
        if (x.length == 0)
            return false;
        if (x.length > 1 && x.charCodeAt(0) == 48)
            return false;
        for (i in 0...x.length) {
    ...
    static function validIdentifier(x:String):Bool {
        if (x.length == 0)
            return false;
        for (i in 0...x.length) {
            var c = x.charCodeAt(i);
    ...
    static function validIdentifiers(x:String):Bool {
        var a = x.split(".");
        for (i in 0...a.length)
            if (!validIdentifier(a[i]))
                return false;
        return true;
    }
    ...
    static function parseFault(version:String):Null<SemverFault> {
        ...
        if (plus >= 0) {
            if (dash >= 0 && plus < dash)
                return InvalidExtension(version);
            if (!validIdentifiers(version.substring(plus + 1)))
                return InvalidExtension(version);
            core = version.substring(0, plus);
        }
        if (dash >= 0) {
            var end = plus >= 0 ? plus : version.length;
            if (!validIdentifiers(version.substring(dash + 1, end)))
                return InvalidExtension(version);
            core = version.substring(0, dash);
        }
        var parts = core.split(".");
        if (parts.length != 3 || !validNumeric(parts[0]) || !validNumeric(parts[1]) || !validNumeric(parts[2]))
            return InvalidCore(version);
        return null;
    }
    ...
    static function parseUnvalidated(version:String):Version {
        ...
        if (dash >= 0)
            pre = splitParts(version.substring(dash + 1, end));
        var core = version;
        if (dash >= 0)
            core = version.substring(0, dash);
        else if (plus >= 0)
    ...
    public static function parse(version:String):Null<Version> {
        var f = parseFault(version);
        if (f != null)
            return null;
        return parseUnvalidated(version);
    }

    public static function require(version:String):Version {
        var f = parseFault(version);
        if (f != null)
            throw exceptionOf(f);
        return parseUnvalidated(version);
    ...
    public static function isPrerelease(version:String):Bool {
    ...
    static function pre(a:Array<String>, b:Array<String>):Int {
        ...
        for (i in 0...n) {
            final x = a[i], y = b[i], xd = validNumeric(x), yd = validNumeric(y);
            if (xd && yd) {
                final d = intOf(x) - intOf(y);
                if (d != 0)
                    return d;
            } else if (xd != yd)
                return xd ? -1 : 1;
