...
class Json {
    ...
    public static function writeCompact(value:JsonValue):String {
        return renderCompact(value);
    }

    static function renderCompact(v:JsonValue):String
        return switch (v) {
            case JNull: "null";
    ...
    static function compactObject(fs:Array<JsonField>):String {
        ...
        for (i in 0...fs.length)
            parts.push(quote(fs[i].name) + ":" + renderCompact(fs[i].value));
        return "{" + parts.join(",") + "}";
    }

    static function indent(n:Int):String {
        var s = "";
        for (i in 0...n)
            s += "  ";
        return s;
    ...
    static function hexDigit(v:Int):String
        return String.fromCharCode(v < 10 ? 48 + v : 87 + v);
    ...
    static function render(v:JsonValue, level:Int):String
        return switch (v) {
            case JNull: "null";
            case JBool(b): if (b) "true" else "false";
            case JNumber(n): Std.string(n);
    ...
    static function renderArray(a:Array<JsonValue>, level:Int):String {
        ...
        final parts = new Array<String>();
        for (i in 0...a.length)
            parts.push(indent(level + 1) + render(a[i], level + 1));
        return "[\n" + parts.join(",\n") + "\n" + indent(level) + "]";
    }
    ...
    static function renderObject(fs:Array<JsonField>, level:Int):String {
        ...
        final parts = new Array<String>();
        for (i in 0...fs.length)
            parts.push(indent(level + 1) + quote(fs[i].name) + ": " + render(fs[i].value, level + 1));
        return "{\n" + parts.join(",\n") + "\n" + indent(level) + "}";
    }
}
...
private class Reader {
    ...
    public function parse():JsonValue {
        ...
        skip();
        if (p != text.length)
            throw new JsonException(TrailingInput(p));
        return v;
    }
    ...
    function parseObject():JsonValue {
        p = p + 1;
        var fs = new Array<JsonField>();
        skip();
        if (takeCode(125))
            return JObject(fs);
        while (true) {
            skip();
            if (text.charCodeAt(p) != 34)
                fail();
            final n = string();
            skip();
            if (!takeCode(58))
                fail();
            fs.push({name: n, value: value()});
            skip();
            if (takeCode(125))
                return JObject(fs);
            if (!takeCode(44))
    ...
    function number():JsonValue {
        var start = p;
        while (p < text.length && isTerm(text.charCodeAt(p)) == false)
            p = p + 1;
        final n = Std.parseFloat(text.substring(start, p));
        if (Math.isNaN(n))
            fail();
        return JNumber(n);
    }

    static function isTerm(k:Int):Bool
        return k == 44 || k == 93 || k == 125 || k == 32 || k == 9 || k == 10 || k == 13;

    function word(w:String):Void {
        if (text.substring(p, p + w.length) != w)
            fail();
        p = p + w.length;
    }

    function skip():Void
        while (p < text.length && isSpace(text.charCodeAt(p)))
            p = p + 1;

    static function isSpace(k:Int):Bool
        return k == 32 || k == 9 || k == 10 || k == 13;

    function takeCode(k:Int):Bool {
        if (text.charCodeAt(p) == k) {
            p = p + 1;
            return true;
        }
        return false;
    ...
    function fail():Void
        throw new JsonException(InvalidJson);
}
