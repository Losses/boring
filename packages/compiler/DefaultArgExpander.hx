...
class DefaultArgExpander {
    ...
    static function discoverNormalizationBindings(args:Array<FunctionArg>, body:Null<Expr>, classType:ClassType,
            registered:Array<Null<CoalescingDefaultValue>>):Void {
        if (body == null)
            return;
            ...
            case ExprDef.ETernary(condition, _, _):
                ...
                switch (c == null ? null : c.expr) {
                    case ExprDef.EBinop(Binop.OpEq, left, _):
                        switch (unwrapExpr(left).expr) {
                            case ExprDef.EConst(AstConstant.CIdent(n)): n;
                            default: null;
                        }
                    default: null;
                }
    ...
    public static function isNormalizationSource(pos:Position):Bool {
        ...
        for (range in normalizationSourceRanges)
            if (range.file == infos.file && infos.min == range.min && infos.max == range.max)
                return true;
        return false;
    }

    static function discoverCoalescingDefaults(args:Array<FunctionArg>, body:Null<Expr>, classType:ClassType,
            ?outSites:Array<CoalescingSiteInfo>):Array<Null<CoalescingDefaultValue>> {
        ...
        for (argIdx in 0...args.length) {
            ...
            if (arg.opt != true || arg.value != null)
                continue;
            ...
            if (candidates.length == 1) {
                registered[argIdx] = {
                    ...
                    defaultExpr: candidates[0].defaultExpr
                };
    ...
    static function collectCoalescingCandidates(e:Expr, parameterName:String, classType:ClassType, earlierNames:Array<String>, allParamNames:Array<String>,
            result:Array<{
            value:CoalescingDefaultValue,
            excluded:Expr,
            defaultExpr:Expr
        }>):Void {
        if (e == null)
    ...
    static function isThisField(e:Expr, fieldName:String):Bool {
        ...
        if (cur == null)
            return false;
        return switch (cur.expr) {
            case ExprDef.EField(receiver, name): name == fieldName && isThisExpression(receiver);
            default: false;
        };
    }
    ...
    static function matchCoalescingExpression(e:Expr, parameterName:String, classType:ClassType, earlierNames:Array<String>,
            allParamNames:Array<String>):Null<{value:CoalescingDefaultValue, defaultExpr:Expr}> {
        final cur = unwrapExpr(e);
        if (cur == null)
            return null;
        return switch (cur.expr) {
    ...
    static function isInstanceField(classType:ClassType, name:String):Bool {
        ...
        if (registered != null && registered.exists(name))
            return true;
        ...
        if (local != null)
            for (field in local.get().fields.get())
                if (field.name == name)
                    return true;
        return false;
    }
     ...
     */
    static function validateCoalescingGrammar(e:Expr, parameterName:String, earlierNames:Array<String>, allParamNames:Array<String>,
            classType:ClassType):Null<CoalescingDefaultValue> {
        final cur = unwrapExpr(e);
        if (cur == null)
            return null;
        ...
        if (cur.expr != null) {
            switch (cur.expr) {
                case ExprDef.EConst(AstConstant.CIdent(name)):
                    ...
                    if (allowNormalizationBindingLeaves && normalizationEarlierLocals.indexOf(name) >= 0)
                        return CLocalRead(name);
                    if (isInstanceField(classType, name) && allowNormalizationBindingLeaves)
                        return CInstanceFieldRead(name);
        ...
        if (allowNormalizationBindingLeaves)
            switch (cur.expr) {
                case ExprDef.EConst(AstConstant.CIdent(name)) if (isInstanceField(classType, name)):
                    return CInstanceFieldRead(name);
                case ExprDef.EField(inner, name) if (allowNormalizationBindingLeaves && isThisExpression(inner) && isInstanceField(classType, name)):
                    return CInstanceFieldRead(name);
                default:
            }
        ...
        if (cur.expr != null) {
            switch (cur.expr) {
                case ExprDef.EField(inner, fieldName):
                    if (allowNormalizationBindingLeaves && isThisExpression(inner) && isInstanceField(classType, fieldName))
                        return CInstanceFieldRead(fieldName);
                    final innerClass = classifyExpr(inner, earlierNames);
                    if (innerClass == "type") {
                            ...
                            case ExprDef.EField(innerClassField, methodName):
                                ...
                                if (receiverClass != "type") {
                                    ...
                                    if (receiverValue != null) {
                                        final argValues = validateArgList(callArgs, parameterName, earlierNames, allParamNames, classType);
                                        if (argValues != null)
                                            return CMethodCall(receiverValue, methodName, argValues);
                                    }
                                }
    ...
    **/
    ...
    static function validateArgList(args:Array<Expr>, parameterName:String, earlierNames:Array<String>, allParamNames:Array<String>,
            classType:ClassType):Null<Array<CoalescingDefaultValue>> {
        ...
        return switch (cur.expr) {
            ...
            case ExprDef.ENew(typePath, params):
                (typePath.pack.length == 0
                    && (typePath.name == "Map" || typePath.name == "StringMap")
                    && params.length == 0) ? CEmptyMap : null;
            case ExprDef.EField(receiver, fieldName):
                if (exprToDotted(receiver) == "Math") {
                    if (fieldName == "POSITIVE_INFINITY")
                        return CPositiveInfinity;
                    if (fieldName == "NEGATIVE_INFINITY")
                        return CNegativeInfinity;
                }
    ...
    static function isNullCheck(e:Expr, parameterName:String):Bool {
        ...
        if (cur == null)
            return false;
        return switch (cur.expr) {
            case ExprDef.EBinop(Binop.OpEq, left, right): isParameterExpression(left, parameterName) && isNullExpression(right);
            default: false;
        };
    }
    ...
    static function coalescingValue(e:Expr, classType:ClassType):CoalescingDefaultValue {
        ...
        switch (cur.expr) {
            ...
            case ExprDef.EArrayDecl(values):
                ...
                coalescingError(cur.pos);
            case ExprDef.ENew(typePath, params):
                if (typePath.pack.length == 0 && typePath.name == "Map" && params.length == 0)
                    return CEmptyMap;
                coalescingError(cur.pos);
            case ExprDef.EField(receiver, fieldName):
                if (exprToDotted(receiver) == "Math") {
            ...
            if (defVal != null) {
                switch (defVal) {
                    case VCoalescing(coalescing):
                        if (!kotlinTarget || !readsParameter(coalescing))
     ...
     */
    static function uniqueByName(counts:Map<String, Int>, byName:Map<String, Array<Null<DefaultArgValue>>>, name:String, kind:String,
            pos:Position):Null<Array<Null<DefaultArgValue>>> {
    ...
    **/
    ...
    public static function coalescingDefaultForLocalParam(classType:ClassType, fieldName:String, localName:String,
            parameterName:String):Null<CoalescingDefaultValue> {
        if (classType == null)
            return null;
    ...
    static function lookupFieldCoalescing(cls:ClassType, fieldName:String, parameterName:String):Null<CoalescingDefaultValue> {
        ...
        if (fieldCoalescing.exists(key))
            return fieldCoalescing.get(key);
        for (iface in cls.interfaces) {
    ...
    public static function hasNestedConditional(e:TypedExpr):Bool {
        ...
        function visit(x:TypedExpr):Void {
            if (x == null || found)
                return;
            switch (x.expr) {
                case TypedExprDef.TIf(_, _, _):
                    found = true;
                default:
                    haxe.macro.TypedExprTools.iter(x, visit);
            }
        }
    ...
    public static function isCoalescingShape(e:TypedExpr):Bool {
        ...
        if (cur == null)
            return false;
        return switch (cur.expr) {
            case TypedExprDef.TIf(condition, _, ifFalse): final parameter = typedNullCheckParameter(condition); parameter != null && isTypedParameter(ifFalse,
                    parameter);
            default: false;
        };
    }
    ...
    static function collectTypedCoalescingSites(e:TypedExpr, result:Array<{parameter:String, defaultExpr:TypedExpr, valueExpr:TypedExpr}>,
            includeNestedFunctions:Bool):Void {
        if (e == null)
            return;
