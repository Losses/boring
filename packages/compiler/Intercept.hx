...
class Intercept {
            ...
            case ExprDef.EField(subject, field):
                switch (subject.expr) {
                    case ExprDef.EConst(Constant.CIdent("StringTools")):
                        if (field == "fastCodeAt") {
                            violation("V18", "NonAsciiStringIndex", "StringTools.fastCodeAt reads storage units with no content meaning; use std.UString.at",
                                subject.pos);
                        }
                        if (field == "fromCharCode") {
                            violation("V18", "NonAsciiStringIndex", "StringTools.fromCharCode writes one utf-16 code unit; use std.UString.fromCodePoint",
                                subject.pos);
                        }
                    default:
                }
    ...
    static function walkSource(e:Expr):Void {
        switch (e.expr) {
            ...
            case ExprDef.EUntyped(_):
                violation("V05", "DynamicValue", "untyped block; the translatable subset declares types", e.pos);
            case ExprDef.ECast(_, null):
                violation("V05", "DynamicValue", "cast without a target type; the translatable subset declares types", e.pos);
            default:
        }
    ...
    static function checkAssignArgExpression(callee:Expr, args:Array<Expr>):Void {
        ...
        for (index in 0...args.length) {
            ...
            if (isAssignArg(arg)) {
                violation("V17", "AssignArgExpression", "assignment expressions in call arguments belong to the record copy construct only", arg.pos);
            }
        }
    }
            ...
            case ExprDef.EField(receiver, name):
                if (name == "allEnums" || name == "enumConstructor" || name == "createEnum") {
                    switch (receiver.expr) {
                        case ExprDef.EConst(Constant.CIdent("Type")):
                            if (name == "createEnum" && args.length != 2)
                                Context.fatalError("Type.createEnum accepts the two-argument form only", callee.pos);
                            if (name == "allEnums" && (args.length != 1 || switch (args[0].expr) {
                                case ExprDef.EConst(Constant.CIdent(_)): false;
                                case _: true;
                            }))
                                Context.fatalError("Type.allEnums accepts an enum type reference only", callee.pos);
                            if (name == "createEnum" && switch (args[0].expr) {
                                    case ExprDef.EConst(Constant.CIdent(_)): false;
                                    case _: true;
                                })
                                Context.fatalError("Type.createEnum accepts the two-argument form only", callee.pos);
                ...
                if (name == "sort") {
                    if (args.length > 0) {
                        violation("V02", "FunctionalIteration",
                            "comparator sort rewrites to a plain loop before translation; the exit is the sort runtime of features/17", callee.pos);
                    }
                    return;
                }
                ...
                for (index in 0...FUNCTIONAL_METHODS.length) {
                    if (name == FUNCTIONAL_METHODS[index]) {
                        violation("V02", "FunctionalIteration", '.$name rewrites to a plain loop before translation', callee.pos);
                    }
                }
                switch (receiver.expr) {
                    case ExprDef.EConst(Constant.CIdent(root)):
                        for (index in 0...REFLECTION_ROOTS.length) {
                            if (root == REFLECTION_ROOTS[index]) {
                                violation("V03", "Reflection", '$root.$name has no translation with identical behavior', callee.pos);
                            }
                        }
                    default:
                }
                ...
                switch (binaryOperator) {
                    case Binop.OpAssign | Binop.OpAssignOp(_):
                        switch (target.expr) {
                            ...
                            case TypedExprDef.TField(_,
                                FieldAccess.FStatic(classRef, fieldRef)) | TypedExprDef.TField(_, FieldAccess.FInstance(classRef, _, fieldRef)):
    ...
    static function checkIterationSubject(subject:TypedExpr):Void {
        ...
        if (!sanctioned) {
            violation("V01", "IteratorLoop", "for subject is not an integer range; arrays iterate as for (i in 0...array.length)", subject.pos);
        }
    }
    ...
    static function walk(e:TypedExpr, inLoop:Bool):Void {
        ...
        final skipDynamicCheck = switch (e.expr) {
            case TypedExprDef.TThrow(_) | TypedExprDef.TReturn(_) | TypedExprDef.TBreak | TypedExprDef.TContinue | TypedExprDef.TBlock(_) | TypedExprDef.TCast(_,
                _):
                true;
            case TypedExprDef.TCall(callee, _): isSyntaxPlumbingCall(callee) || isNestedSyntaxPlumbingCall(callee);
            default:
                false;
        };
        if (!skipDynamicCheck) {
     ...
     */
    static function checkRegion(region:TypedExpr, body:TypedExpr, catches:Array<{v:TVar, expr:TypedExpr}>, inLoop:Bool):Void {
        if (catches.length != 1) {
            violation("V20", "TryRegionMixedDomains", "try region handles exactly one exception domain; nest one region per domain", region.pos);
        } else {
                    ...
                    case Type.TInst(classRef, _):
                        ...
                        if (module != caughtModule && absorbed.indexOf(module) < 0) {
                            violation("V20", "TryRegionMixedDomains",
                                "try region body throws " + classRef.get().name + " beyond the caught class; nest one region per domain", e.pos);
                        }
                    default:
    ...
    static function checkRegionControlFlow(e:TypedExpr, inLoop:Bool):Void {
        switch (e.expr) {
            case TypedExprDef.TReturn(_):
                violation("V19", "TryRegionControlFlow", "return inside a try region body; hoist the region and return after it", e.pos);
            case TypedExprDef.TBreak if (!inLoop):
                violation("V19", "TryRegionControlFlow", "break inside a try region body; hoist the region and break after it", e.pos);
            case TypedExprDef.TContinue if (!inLoop):
                violation("V19", "TryRegionControlFlow", "continue inside a try region body; hoist the region and continue after it", e.pos);
            case TypedExprDef.TWhile(condition, body, _):
    ...
    static function checkStringKeyedAccess(subject:TypedExpr, index:TypedExpr):Void {
        ...
        switch (subject.t) {
            case Type.TAnonymous(_):
                violation("V06", "StringKeyedAccess", "field access on a structure uses the dot form; brackets are for integer indices", index.pos);
            default:
        }
    }
    ...
    static function checkFinalAssign(target:TypedExpr):Void {
        switch (target.expr) {
            case TypedExprDef.TField(_, FieldAccess.FAnon(fieldRef)) | TypedExprDef.TField(_, FieldAccess.FInstance(_, _, fieldRef)):
                if (fieldRef.get().isFinal && !inConstructorBody) {
                    violation("V07", "ShapeMutation", "assignment targets a final field", target.pos);
                }
            default:
        }
    }
    ...
    static function checkDynamic(e:TypedExpr):Void {
        switch (e.t) {
            case Type.TDynamic(_):
                violation("V05", "DynamicValue", "expression is typed Dynamic; the translatable subset declares types", e.pos);
            default:
        }
    }
    ...
    static function isStringType(t:Type):Bool {
        return switch (t) {
            case Type.TInst(classRef, _): final classType = classRef.get(); classType.pack.length == 0 && classType.name == "String";
            default:
                false;
        };
    }
    ...
    static function isStringFromCharCode(fieldAccess:FieldAccess):Bool {
        return switch (fieldAccess) {
            case FieldAccess.FStatic(classRef, fieldRef): final classType = classRef.get(); classType.pack.length == 0 && classType.name == "String" && fieldRef.get()
                    .name == "fromCharCode";
            default:
                false;
        };
    ...
    static function checkAsciiIndexSubject(subj:TypedExpr, op:String):Void {
        ...
        if (literal != null && !isAsciiOnly(literal)) {
            violation("V18", "NonAsciiStringIndex", "string index access requires ascii content; non-ascii content uses std.UString: " + op, subj.pos);
        }
    }
    ...
    static function stringLiteralOf(subj:TypedExpr):Null<String> {
        switch (subj.expr) {
            ...
            case TypedExprDef.TField(_, FieldAccess.FStatic(classRef, fieldRef)) | TypedExprDef.TField(_, FieldAccess.FInstance(classRef, _, fieldRef)):
    ...
    static function checkFromCharCodeArgument(arg:TypedExpr):Void {
        switch (arg.expr) {
            case TypedExprDef.TConst(TInt(code)):
                if (code < 0 || code > 0xFF) {
                    violation("V18", "NonAsciiStringIndex", "String.fromCharCode accepts wire bytes 0..255; code points use std.UString.fromCodePoint",
                        arg.pos);
                }
            default:
        }
        ...
        switch (e.t) {
            case Type.TAbstract(abstractRef, _):
                if (moduleBanned(abstractRef.get().module)) {
                    violation("V13", "HashMapCollection", "haxe.ds.Map and its implementations have no translation", e.pos);
                }
            case Type.TInst(classRef, _):
    ...
    static function checkEnumDefault(subject:TypedExpr, maybeDefault:Null<TypedExpr>):Void {
        ...
        if (isEnumSwitch) {
            violation("V15", "EnumDefaultArm", "switch over an enum carries a default arm; enum switches list every variant", maybeDefault.pos);
        }
    }
    ...
    static function checkInt64(e:TypedExpr):Void {
        ...
        switch (e.expr) {
            case TypedExprDef.TBinop(op, left, right):
                if (isInt64Type(left.t) || isInt64Type(right.t))
                    switch (op) {
                        case OpAssign | OpAssignOp(_) | OpAdd | OpSub | OpAnd | OpOr | OpXor | OpShl | OpShr | OpUShr | OpEq | OpNotEq | OpLt | OpGt | OpLte |
                            OpGte:
                        default:
                            violation("V11", "Int64Misuse", "Int64 operation has no lowering in the translatable subset", e.pos);
                    }
            case TypedExprDef.TCall(callee, args):
                if (isStdStringCall(callee) && hasInt64Argument(args)) {
                    violation("V11", "Int64Misuse", "Std.string has no Int64 lowering in the translatable subset", e.pos);
                }
            default:
        }
    }
    ...
    static function isStdStringCall(callee:TypedExpr):Bool {
        return switch (callee.expr) {
            case TypedExprDef.TField(_, FieldAccess.FStatic(classRef, fieldRef)): classRef.get().module == "Std" && fieldRef.get().name == "string";
            default: false;
        };
    }
