...
class RuntimeConfig {
    ...
    **/
    public static function requireImportName(what:String):String {
        ...
        if (name == null) {
            Context.error("runtime-import define is required to reference the runtime package ("
                + what
                + "); the consumer's package identity is not derivable from the compilation and no default exists",
                Context.currentPos());
        }
        return name;
    }
