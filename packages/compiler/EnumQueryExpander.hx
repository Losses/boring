...
class EnumQueryExpander {
    ...
    public static function usage(en:EnumType):Null<{
        collection:Bool,
        name:Bool,
        lookup:Bool,
        en:EnumType
    ...
    public static function markerInner(e:TypedExpr):TypedExpr
        return switch (e.expr) {
    ...
    public static function aliasEnum(e:TypedExpr):Null<EnumType>
        return switch (e.expr) {
            case TLocal(v): aliases.get(v.id);
    ...
    public static function aliasById(id:Int):Null<EnumType>
        return aliases.get(id);

    public static function collectionEnum(e:TypedExpr):Null<EnumType> {
        if (markerKind(e) == QCollection)
            return enumOf(e);
        return aliasEnum(e);
    }

    public static function callArgs(e:TypedExpr):Array<TypedExpr>
        return switch (markerInner(e).expr) {
    ...
    static function walk(e:TypedExpr, position:String):Void {
        ...
        if (all != null) {
            ...
            if (position != "length" && position != "index" && position != "alias")
                Context.fatalError("Type.allEnums expands in length and index positions only", e.pos);
        ...
        switch (e.expr) {
            case TVar(_, init):
                if (init != null)
                    walk(init, allEnumsType(init) != null ? "alias" : "other");
            case TField(subj, fa):
                ...
                if (name == "length" && (allEnumsType(subj) != null || aliasEnum(subj) != null))
                    walk(subj, "length");
                else
                    walk(subj, "other");
            case TArray(subj, index):
                if (allEnumsType(subj) != null || aliasEnum(subj) != null)
                    walk(subj, "index");
                else
                    walk(subj, "other");
                walk(index, "other");
            case TCall(callee, args):
                ...
                switch (member) {
                    case "enumConstructor":
                        if (args.length != 1)
                            Context.fatalError("Type.enumConstructor accepts parameterless enum values only", e.pos);
                        ...
                        mark(e, "name");
                    case "createEnum":
                        if (args.length != 2 && !(args.length == 3 && switch (args[2].expr) {
                            case TConst(TNull): true;
                            case _: false;
                        }))
                            Context.fatalError("Type.createEnum accepts the two-argument form only", e.pos);
                        final en = enumTypeRef(args[0]);
                        if (en == null)
                            Context.fatalError("Type.createEnum accepts the two-argument form only", args[0].pos);
                        ensureValueEnum(en, args[0].pos);
                        walk(args[1], "other");
                        register(en, QLookup);
                        mark(e, "lookup");
            ...
            default:
                haxe.macro.TypedExprTools.iter(e, child -> walk(child, "other"));
        }
    }
    ...
    static function typeMember(e:TypedExpr):Null<String>
        return switch (e.expr) {
            case TField(_, FStatic(cls, field)) if (cls.get().name == "Type"): field.get().name;
            case _: null;
        };

    static function allEnumsType(e:TypedExpr):Null<EnumType>
        return switch (e.expr) {
            ...
            case TCall(callee, args) if (typeMember(callee) == "allEnums"):
                if (args.length != 1)
                    Context.fatalError("Type.allEnums accepts an enum type reference only", e.pos);
                final en = enumTypeRef(args[0]);
                if (en == null)
                    Context.fatalError("Type.allEnums accepts an enum type reference only", args[0].pos);
                en;
            case _: null;
        };

    static function enumTypeRef(e:TypedExpr):Null<EnumType>
        return switch (e.expr) {
    ...
    static function enumFromType(t:Type):Null<EnumType>
        ...
        for (ef in en.constructs)
            switch (Context.follow(ef.type)) {
                case TFun(args, _) if (args.length > 0):
                    Context.fatalError("enum queries accept enums without constructor parameters only", pos);
                case _:
            }
    }
}
