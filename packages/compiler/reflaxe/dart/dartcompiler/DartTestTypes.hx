...
/**
    ...
    (docs/specs/features/19-testing.md). Scalar arguments route to
    TestCore directly; every other argument type registers here, and the
    output stage generates the canonical formatter, the deep equality,
    and the assertion function for it. The tags spell the Dart shapes
    (Int, Double, Bool, String, OptItem, ItemArray) so the generated
    function names read without decoding.
**/
class DartTestTypes {
    ...
    /**
        The generated-name tag of a test value type. Typedefs and the
        special abstracts keep their identity: `Context.follow` would
        unwrap a named record to its anonymous structure and lose the
        name, so only lazy wrappers resolve here.
    **/
    ...
    **/
    public static function isScalarRoute(t:Type):Bool {
        return switch (t) {
            case TAbstract(a, _): final name = a.get().name; name == "Int" || name == "Float" || name == "Bool";
            case TInst(c, _):
                c.get().name == "String";
    ...
    **/
    ...
    static function registerInto(t:Type):Void {
        ...
        switch (t) {
            ...
            case TAnonymous(an):
                ...
                final def = DartDecl.structTypedefs.get(DartDecl.structureSignature(an));
                if (def != null) {
    ...
    static function fail(t:Type):String {
        Context.error("test assertion type has no Dart lowering in the subset: " + Std.string(t), Context.currentPos());
        return "";
    }
}
#end
