...
class DartExpr {
    ...
    **/
        ...
        return switch (value) {
            ...
            case CEmptyMap:
                final mapParams = switch (DefaultArgExpander.withoutNull(targetType)) {
                    ...
                    case TAbstract(def, params) if (def.get().pack.join(".") == "haxe.ds"
                        && def.get().name == "Map"
                        && params.length == 2): params;
                    case _: [];
                };
                mapParams.length == 2 ? "<" + types.of(mapParams[0]) + ", " + types.of(mapParams[1]) + ">{}" : "<dynamic, dynamic>{}";
            ...
            case CParameterRead(name):
                ...
                final earlier = currentClass != null
                    && currentField != null ? (currentLocalName != null ? DefaultArgExpander.coalescingDefaultForLocalParam(currentClass, currentField,
                        currentLocalName, name) : DefaultArgExpander.coalescingDefaultForParam(currentClass, currentField, name)) : null;
    ...
    function coalescingStaticCallText(path:String, args:Array<DefaultArgExpander.CoalescingDefaultValue>, targetType:Type):String {
        ...
        if (path == "std.SortedSet.builder")
            return runtimeQualified("SortedTable.setBuilder") + "(" + rendered + ")";
    ...
    /**
        ...
        constructor body).
    **/
    public function constructorParts(cls:ClassType, f:ClassFuncData):{
        formalFields:Map<String, String>,
        fieldInits:Array<String>,
        superCall:Null<String>,
        body:Array<String>
    } {
        if (f.expr == null) {
                    ...
                    switch (stripWrap(target).expr) {
                        case TField({expr: TConst(TThis)}, FInstance(owner, _, cf)):
                            ...
                            if (param != null) {
                                ...
                                if (fields == null) {
                                    fields = [];
                                    parameterFields.set(param, fields);
                                }
        ...
        while (statementIndex < statements.length) {
            ...
            final fused = fillFusion(fillStatements, fillIndex, 2, packedLoop ? [
                statements[statementIndex + 1],
                statements[statementIndex + 2],
                statements[statementIndex + 3]
            ] : null);
            if (fused != null) {
                for (line in terminated(fused))
                    body.push(line);
    ...
    **/
    ...
    function mentionsConstructorLocalExpr(e:TypedExpr, locals:Map<String, Bool>):Bool {
        ...
        function walk(x:TypedExpr):Void {
            switch (x.expr) {
                case TLocal(v) if (locals.exists(v.name)):
                    found = true;
                case _:
            }
            if (!found)
    ...
    **/
    function fillFusion(stmts:Array<TypedExpr>, i:Int, depth:Int, flatLoop:Null<Array<TypedExpr>> = null):Null<Array<String>> {
        ...
        if (flatLoop != null) {
            final counter = switch (flatLoop[0].expr) {
                case TVar(v, _): v;
                case _: return null;
            };
            final boundVar = switch (flatLoop[1].expr) {
                case TVar(v, _): v;
                case _: return null;
            };
            final start = switch (flatLoop[0].expr) {
                case TVar(_, value): value;
                case _: return null;
            };
            final bound = switch (flatLoop[1].expr) {
                case TVar(_, value): value;
                case _: return null;
            };
    ...
    **/
    ...
    function functionLiteral(f:TFunc):String {
        ...
        for (a in f.args) {
            final coalescing = currentLocalName != null
                && currentClass != null
                && currentField != null
        ...
        if (optional.length > 0)
            paramGroups.push("[" + optional.join(", ") + "]");
    ...
    **/
    ...
    function stdString(arg:TypedExpr, inConcat:Bool):String {
        return stdStringType(arg.t, nullableCollectionSubject(arg), inConcat, arg);
    }

    /**
        The subject text of a Std.string operand. The record member
        guards a nullable collection field with an explicit null
        comparison before the call; Dart cannot promote a public field
        read through the guard or into the collection closure, so the
        subject force-unwraps when its type is Null over a collection.
    **/
    function nullableCollectionSubject(arg:TypedExpr):String {
        if (!isNullableCollectionType(arg.t)) {
            return expr(arg);
        }
        return "(" + expr(arg) + ")!";
    }
    ...
    **/
    function enumLabeledText(en:EnumType, value:String, origin:TypedExpr):String {
        ...
        for (ef in constructs) {
            ...
            final pattern = qualifiedRef(en.module, cls)
                + (args.length > 0 ? "(" + [for (arg in args) arg.name + ": var " + arg.name].join(", ") + ")" : "()");
            if (args.length == 0) {
    ...
    function valueTypeCall(fn:TypedExpr, args:Array<TypedExpr>):Null<String> {
        switch (stripWrap(fn).expr) {
            case TField(_, FStatic(c, cf)):
                ...
                if (op != null) {
                    return switch (op) {
                        case Binary(_): args.length >= 2 ? expr(args[0]) + " " + opStrForValue(op) + " " + expr(args[1]) : qualifiedRef(c.get().module,
                                abs.name);
            ...
            case TField(subj, FStatic(c, cf)):
                ...
                if (module == "Math") {
                    ...
                    switch (fName) {
                        ...
                        case "round": return "(" + expr(args[0]) + ").round()";
        ...
        switch (fn.expr) {
            ...
            case TField(subj, FInstance(owner, _, cf)):
                ...
                if (name == "blit" && args.length == 4 && isBytes(stripCast(subj).t)) {
                    return receiverText(subj) + ".setRange(" + expr(args[0]) + ", " + expr(args[0]) + " + " + expr(args[3]) + ", " + expr(args[1])
                        + ".sublist(" + expr(args[2]) + ", " + expr(args[2]) + " + " + expr(args[3]) + "))";
                }
    ...
    function ustringPlatformCall(fName:String, args:Array<TypedExpr>, fn:TypedExpr):String {
        ...
        switch (fName) {
            ...
            case "fromCodePoint":
                ...
                return "((" + cp + " >= 0 && " + cp + " <= 1114111 && !(" + cp + " >= 55296 && " + cp + " <= 57343)) ? String.fromCharCode(" + cp
        ...
        switch (fName) {
            ...
            case "equals":
                ...
                if (DartTestTypes.isScalarRoute(t)) {
                    return switch (t) {
                        case TAbstract(a, _) if (a.get().name == "Int"): core
                            + ".equalsInt("
                            + expr(args[0])
                            + ", "
                            + expr(args[1])
                            ...
                            + message
                            + ")";
                        case TAbstract(a, _) if (a.get()
                            .name == "Float"): core + ".equalsFloat(" + expr(args[0]) + ", " + expr(args[1]) + ", " + message + ")";
                        case TAbstract(a, _) if (a.get().name == "Bool"): core
                            + ".equalsBool("
                            + expr(args[0])
                            + ", "
                            + expr(args[1])
    ...
    function sortedByCall(args:Array<TypedExpr>, fn:TypedExpr):String {
        ...
        if (func != null && func.args.length == 1) {
            ...
            final comparator = isStringLeafType(bodyExpr.t)
                || isIntLeafType(bodyExpr.t) ? keyA + ".compareTo(" + keyB + ")" : "compare"
                + structKeyTagName(bodyExpr.t)
                + "("
                + keyA
                + ", "
                + keyB
    ...
    function newExpr(c:Ref<ClassType>, params:Array<Type>, args:Array<TypedExpr>):String {
        ...
        if (valueType != null) {
            return ValueTypeSupport.constructorThrows(valueType) ? qualifiedRef(cls.module, ValueTypeSupport.constructorName(valueType))
                + "("
                + rendered
                + ")" : qualifiedRef(cls.module, valueType.name)
                + "("
                + rendered
                + ")";
        }
    ...
    **/
    function tryBindingLines(v:TVar, region:TypedExpr, depth:Int):Array<String> {
        ...
        final out = [indent(depth) + types.of(v.t) + " " + name, indent(depth) + "try {"];
    ...
    **/
    ...
    function stringBufTailLines(subj:TypedExpr, depth:Int):{name:String, lines:Array<String>} {
        ...
        return {
            name: name,
            lines: [
                indent(depth) + "final " + name + " = " + buf + ".length > 0 ? " + buf + "[" + buf + ".length - 1] : -1"
            ]
        };
    }
    ...
    function stringBufFaultThrow(depth:Int, unit:String):String {
        return indent(depth)
            + "throw "
            + qualifiedRef("std.UStringException", "UStringException")
            + "("
            + qualifiedRef("std.UStringFault", DartDecl.constructClassName("UStringFault", "UnpairedSurrogate"))
            + "("
            + unit
            + "))";
    }
    ...
    **/
    ...
    function enumTable(se:TypedExpr):Map<Int, {
        name:String,
        field:EnumField,
        enumName:String,
        module:String,
    ...
    **/
    ...
    function isBytesBuffer(t:Type):Bool {
        return switch (Context.follow(t)) {
            case TInst(c, _): final cls = c.get(); cls.pack.join(".") == "haxe.io" && cls.name == "BytesBuffer";
