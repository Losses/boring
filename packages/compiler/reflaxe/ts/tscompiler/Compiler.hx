    ...
    **/
    ...
    public static function use() {
        ...
        if (FloatPrecision.isF32()) {
            Context.error("float-precision=f32 is not available on the TypeScript target: number is binary64; compile without the define for f64 semantics",
                Context.currentPos());
        }
        ...
        if (RuntimeConfig.importName() != null) {
            // Force every resident module through typing so its declarations
            // are emitted into runtime.ts / runtime/test.ts regardless of whether the
            // consumer's own sources happen to exercise the corresponding
            // extern. Without this, a consumer that only uses a subset of
            // std.UString / std.Graphemes / std.SortedMap never types the
            // resident backing module, so runtime.ts omits UString,
            // Graphemes, SortedMapTable, etc., and the emitted imports from
            // @<consumer>/runtime fail with TS2307/TS2305. The business
            // externs never type the residents (they lower through the
            // runtime-name table), so the only reliable trigger is here.
            for (module in RuntimeResidents.MODULES) {
                Context.getType(module);
            }
            for (module in RuntimeResidents.TEST_MODULES) {
                Context.getType(module);
            }
        }
    ...
    public override function generateFilesManually() {
        ...
        for (module in modules) {
            ...
            if (testModules.exists(module)) {
                final imports = decl.renderTestImports(testOutput, tsOutput, testRunner, testModules);
                final helpers = decl.renderFsHelpers();
        ...
        for (module in ExternBindings.modules()) {
            ...
            PackageArtifacts.saveTreeFile(output, modulePath(module), '// Generated by the reflaxe TypeScript target. Do not edit.\n\n' + bindings + "\n");
        }
        ...
        if (emitDir != null && anyRuntimeUsed()) {
            ...
            PackageArtifacts.saveTreeFile(output, RuntimeConfig.emitPath(emitDir, "runtime.ts"),
                StringTools.trim(TsRuntime.SOURCE)
                + "\n"
                + residentParts.join("\n\n")
                + "\n");
            if (anyRuntimeTestUsed()) {
                ...
                PackageArtifacts.saveTreeFile(output, RuntimeConfig.emitPath(emitDir, "runtime/test.ts"),
                    StringTools.trim(TsRuntime.TEST_SOURCE)
                    + "\n"
    ...
    **/
    function emitPackageShell():Void {
        ...
        if (anyRuntimeUsed() && runtimeImport != null && !TsImports.isRelativeSpecifier(runtimeImport)) {
            Context.error("package shell requires a relative runtime import: a by-name runtime import names a package the manifest cannot declare; pass runtime-import a relative specifier or package-shell none",
                Context.currentPos());
        }
        ...
        lines.push('  }');
        lines.push("}");
        PackageArtifacts.saveTreeFile(output, "package.json", lines.join("\n") + "\n");
    }
