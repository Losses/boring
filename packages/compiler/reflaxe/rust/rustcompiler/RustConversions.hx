...
class RustConversions {
    ...
    **/
    public static function floatToU32(x:String):String {
        return "match f64::from("
            + x
            + ") { v if !(v >= 0.0) || v.is_nan() => 0u32, v if v >= 4294967296.0 => 4294967295u32, v => "
            + truncU32Inner("v")
            + " }";
    }
    ...
    **/
    public static function floatToI32(x:String):String {
        return "match f64::from("
            ...
            + "v if v < 0.0 => i32::from_ne_bytes(u32::wrapping_sub(0, "
            + truncU32Inner("(0.0 - v)")
            + ").to_ne_bytes()), "
            + "v => i32::from_ne_bytes("
            + truncU32Inner("v")
    ...
    static function truncU32Inner(v:String):String {
        return "match (("
            + v
            + ").to_bits() >> 52) & 2047 { e if e < 1023 => 0u32, "
            + "e => u32::try_from(((("
            + v
            + ").to_bits() & 4503599627370495) | 4503599627370496) >> (52 - (e - 1023))).unwrap_or(0) }";
    }
