    ...
    function loopBound(bound:TypedExpr):String {
        ...
        switch (inner.expr) {
            case TField(subj, fa) if (fieldName(fa) == "length"):
                ...
                // a loop bound must unwrap the Option before len(). The
                // narrowing check runs first because Haxe may narrow the
                // field's type to non-null inside the guard while the Rust
                // field still stores Option.
        ...
        switch (fn.expr) {
            ...
            case TField(_, FStatic(c, cf)):
                ...
                if (cls.pack.length == 0
                    ...
                    // receiver maps to false when None. The closure
                    // receives a &String view; the pattern is already
                    // &str from the literal argument.
                    if (isNullType(args[0].t)) {
                        return "(" + expr(args[0]) + ").as_ref().map_or(false, |v| v." + RustImports.toSnakeCase(name) + "(" + expr(args[1]) + "))";
                    }
