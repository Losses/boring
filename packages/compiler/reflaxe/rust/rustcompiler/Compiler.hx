...
class Compiler extends PluginCompiler<Compiler> {
    ...
    public function compileClassImpl(classType:ClassType, varFields:Array<ClassVarData>, funcFields:Array<ClassFuncData>):Null<String> {
        ...
        if (isSyntheticImpl(classType.name)) {
            ...
            // calls needs the `_Impl_` to resolve (features/49). Unreferenced
    ...
    **/
    ...
    public override function generateFilesManually() {
        ...
        for (pack in packages.keys())
            allPackages.set(pack, true);
        for (pack in packageChildren.keys())
            allPackages.set(pack, true);
        ...
        if (emitDir != null && hasAnyShim()) {
            final runtimeMods = [];
            if (state.shimsUsed.exists("haxe.io.FPHelper"))
                    ...
                    for (opt in en.constructs) {
                        switch (Context.follow(opt.type)) {
                            case TFun(args, _):
                                ...
                                final fmtArgs = [
                                    for (arg in args) {final argType = (arg.name == "offset" || arg.name == "length" || arg.name == "remaining") ? "usize" : typeSafeSnake(arg.t,
                                        types);
                                        'format_'
                                        + argType
                                        + '('
                                        + RustImports.toSnakeCase(arg.name)
                                        + ')';
                                    }
                                ].join(' + ", " + ');
    ...
    **/
    ...
    function preScan(mtypes:Array<haxe.macro.Type.ModuleType>):Void {
        ...
        scanInterfaceMethodShapes(mtypes);
        scanStaticReferences(mtypes);
    }

    /**
        Copies concrete implementation shapes onto interface methods. Interface
    ...
    function scanFallibility(mtypes:Array<haxe.macro.Type.ModuleType>):Void {
        ...
        function markRecordCloneArgs(fn:TypedExpr, args:Array<TypedExpr>):Void {
            ...
            for (i in 0...args.length) {
                if (i >= paramTypes.length || isPassByRefType(paramTypes[i]))
                    continue;
                final isLocal = switch (stripDecorations(args[i]).expr) {
                    case TLocal(_): true;
                    case _: false;
                };
                if (isLocal) {
            ...
            switch (mt) {
                case TClassDecl(c):
                    ...
                    for (field in cls.statics.get())
                        scanField(field, true);
                    for (field in cls.fields.get())
                        scanField(field, false);
        ...
        // unions and leaf errors into their already-synthetic callers,
        // guarding against cycles with transitive reachability.
        ...
        // propagation: edges resolved above may have grown a union past the
        // member set it was declared with, and a union shared across several
        // callers (same resolved name, different owning classes) must carry
        // every caller's contribution. Push newly-seen members into the
        // union's permanent member list, skipping the union itself.
        for (entry in entries) {
            final resolved = state.funcErrorTypes.get(entry.key);
            if (resolved == null || !state.isSyntheticErrorType(resolved.name)) continue;
            final set = members.get(entry.key);
            if (set == null) continue;
            final nested = state.syntheticErrorMembers(resolved.name);
            if (nested == null) continue;
            for (item in set) {
                if (item.name == resolved.name) continue;
                var seen = false;
                for (m in nested)
                    if (m.module == item.module && m.name == item.name)
                        seen = true;
        ...
        // Refresh variant tables for every synthetic union whose member set
        // grew above, naming variants by the union's owning class so the
        // prefix-stripping convention stays consistent with declareUnion.
        for (module in state.syntheticErrorEnums.keys()) {
            final kcname = module.substr(module.lastIndexOf(".") + 1);
            for (decl in state.syntheticErrorEnums.get(module)) {
                final variants:Map<String, String> = [];
