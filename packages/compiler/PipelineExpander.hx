...
class PipelineExpander {
    ...
    static function expandArrayIteration(stmt:TypedExpr, usedNames:Map<String, Bool>):Null<Array<TypedExpr>> {
        switch (stmt.expr) {
            case TFor(item, subject, body):
                ...
                if (arraySubject == null || elemType == null)
                    return null;
    ...
    static function expandLoweredReadOnlyIteration(counterStmt:TypedExpr, whileStmt:TypedExpr, usedNames:Map<String, Bool>):Null<Array<TypedExpr>> {
        ...
        switch [counterStmt.expr, whileStmt.expr] {
            case [TVar(iteratorVar, init), TWhile(_, loopBody, true)]:
                switch (init.expr) {
                    case TCall({expr: TField(_, FStatic(c, f))}, args)
                        if (args.length == 1
                            && c.get().name == "ReadOnlyArray_Impl_"
                            && f.get().name == "iterator"
                            && StaticFieldHelper.isReadOnlyArrayType(args[0].t)):
                        arrayExpr = stripWrap(args[0]);
                    default: return null;
                }
                final bodyStmts = switch loopBody.expr {
                    case TBlock(s): s;
                    case _: [];
                };
        ...
        final index = makeTVar(mint("_g_index", usedNames), Context.getType("Int"), true);
        return [
            makeTVarStmt(counter, makeIntConst(0, pos), pos),
            makeTVarStmt(bound, makeArrayLength(arrayExpr, pos), pos),
            makeIntervalLoop(counter, bound, index, [
                makeTVarStmt(item, makeArrayRead(arrayExpr, makeLocal(index, pos), elemType, pos), pos)
            ].concat(tail), pos)
        ];
    }
     ...
     */
    static function expandLoweredArrayIteration(counterStmt:TypedExpr, whileStmt:TypedExpr, subjectStmt:Null<TypedExpr>,
            usedNames:Map<String, Bool>):Null<Array<TypedExpr>> {
            ...
            case [TVar(counterVar, init), TWhile(cond, loopBody, true)]:
                ...
                switch (stripWrap(cond).expr) {
                    case TBinop(OpLt, {expr: TLocal(c)}, {expr: TField(recv, FInstance(_, _, fl))}):
                        if (c.id != counterVar.id || fl.get().name != "length")
                            return null;
                        switch (recv.expr) {
                            case TLocal(r):
                                if (!StaticFieldHelper.isArrayType(r.t))
                                    return null;
    ...
    static function transformExpr(e:TypedExpr, usedNames:Map<String, Bool>):Void {
        ...
        switch (e.expr) {
            ...
            case TIf(cond, ifExpr, elseExpr):
                ...
                if (elseExpr != null)
                    transformExpr(elseExpr, usedNames);
            ...
            case TSwitch(subj, cases, def):
                ...
                for (c in cases) {
                    for (v in c.values)
                        transformExpr(v, usedNames);
                    transformExpr(c.expr, usedNames);
                }
                if (def != null)
                    transformExpr(def, usedNames);
            case TVar(v, init):
                if (init != null)
                    transformExpr(init, usedNames);
            case TReturn(ret):
                if (ret != null)
                    transformExpr(ret, usedNames);
            default:
    ...
    static function transformBlock(stmts:Array<TypedExpr>, usedNames:Map<String, Bool>):Void {
        ...
        while (i < stmts.length) {
            ...
            if (iteration != null) {
                stmts.splice(i, 1);
                for (j in 0...iteration.length)
                    stmts.insert(i + j, iteration[j]);
                i += iteration.length;
                continue;
            }
    ...
    static function transformInnerBlocks(e:TypedExpr, usedNames:Map<String, Bool>):Void {
        ...
        switch (e.expr) {
            ...
            case TIf(cond, ifExpr, elseExpr):
                ...
                if (elseExpr != null)
                    transformInnerBlocks(elseExpr, usedNames);
            case TWhile(cond, body, normalWhile):
                transformInnerBlocks(cond, usedNames);
            ...
            case TSwitch(subj, cases, def):
                ...
                for (c in cases) {
                    for (v in c.values)
                        transformInnerBlocks(v, usedNames);
                    transformInnerBlocks(c.expr, usedNames);
                }
                if (def != null)
    ...
    static function validatePipelinePositions(e:TypedExpr, isDirect:Bool):Void {
        ...
        switch (e.expr) {
            ...
            case TCall(fn, args):
                for (a in args)
                    validatePipelinePositions(a, false);
            ...
            case TArrayDecl(elems):
                for (elem in elems)
                    validatePipelinePositions(elem, false);
            case TObjectDecl(fields):
                for (f in fields)
                    validatePipelinePositions(f.expr, false);
            default:
    ...
    static function isSideEffectFree(e:TypedExpr):Bool {
        ...
        return switch (e.expr) {
            ...
            case TArray(subj, idx): isSideEffectFree(subj) && isSideEffectFree(idx);
            case TBlock(stmts):
                var ok = true;
                for (s in stmts)
                    if (!isSideEffectFree(s))
                        ok = false;
                ok;
            case TVar(v, init): init == null || isSideEffectFree(init);
            case TReturn(ret): ret == null || isSideEffectFree(ret);
            case TBinop(op, e1, e2):
                switch (op) {
            ...
            case TArrayDecl(elems):
                var ok = true;
                for (elem in elems)
                    if (!isSideEffectFree(elem))
                        ok = false;
                ok;
            case TObjectDecl(fields):
                var ok = true;
                for (f in fields)
                    if (!isSideEffectFree(f.expr))
                        ok = false;
                ok;
            default: false;
        };
    ...
    static function parseInlinedArrayMethod(e:TypedExpr):Null<PipelineCall> {
        ...
        switch (e.expr) {
            case TBlock(stmts) if (stmts.length == 3):
                if (stmts[0] == null || stmts[1] == null || stmts[2] == null)
                    return null;
                    ...
                    case TBlock(innerStmts) if (innerStmts.length >= 2):
                        ...
                        switch (whileExpr.expr) {
                            case TWhile(cond, whileBody, _):
                                ...
                                if (receiverExpr == null || !isArrayType(receiverExpr.t))
                                    return null;
    ...
    static function findLambdaInInlinedLoop(whileBody:TypedExpr, receiverExpr:TypedExpr, topBlock:TypedExpr):Null<PipelineCall> {
        if (whileBody == null || !isPipelineSource(topBlock))
            return null;
    ...
    static function isArrayType(t:Type):Bool {
        ...
        return switch (Context.follow(t)) {
            case TInst(c, params): c.get().name == "Array" && c.get().pack.length == 0 && params.length == 1;
            default: false;
        };
    }
        ...
        function checkFun(node:TypedExpr):Void {
            ...
            switch (node.expr) {
                case TFunction(_):
                    hasNestedFun = true;
                default:
                    haxe.macro.TypedExprTools.iter(node, checkFun);
            }
        }
            ...
            case "associate":
                ...
                final bodyStmts = adaptLambdaBody(lambda.expr, entryVal -> {
                    ...
                    if (keyExpr != null && valExpr != null) {
                        ...
                        final putCall = makeSortedMapPutCall(makeLocal(builderVar, pos), makeFieldRead(makeLocal(entryVar, pos), "key", keyType, pos),
                            makeFieldRead(makeLocal(entryVar, pos), "value", valType, pos), pos);
                        {
