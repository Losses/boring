    ...
    **/
    public static function formatFloat(v:Float):String {
        ...
        if (decimalPosition >= -5 && decimalPosition <= 21) {
            var plain = decimalPosition <= 0 ? plainLeading(digits,
                decimalPosition) : decimalPosition >= digits.length ? plainTrailing(digits, decimalPosition) : digits.substring(0, decimalPosition)
                    + "."
                    + digits.substring(decimalPosition);
            while (plain.length > 0 && plain.charCodeAt(plain.length - 1) == 48 && plain.split(".").length > 1)
                plain = plain.substring(0, plain.length - 1);
            if (plain.length > 0 && plain.charCodeAt(plain.length - 1) == 46)
                plain = plain.substring(0, plain.length - 1);
    ...
    static function plainTrailing(digits:String, decimalPosition:Int):String {
        var tail = "";
        for (_ in 0...decimalPosition - digits.length)
            tail += "0";
        return digits + tail;
    }
