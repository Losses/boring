...
- `ts/`: the TypeScript codec package (`@boring/codec`), a member of the
  bun workspace at the root `package.json`. `ts/gen/` holds the tree
  generated by the reflaxe TypeScript target: it is committed output,
  regenerated by `bun run gen:ts`, and `verify` fails when regeneration
