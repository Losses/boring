...
each platform through the reflaxe targets under `tools/reflaxe/`. This
repository hosts the targets, the interception pass, the capability
samples under `haxe/src/`, the hand-written reference translations under
`ts/`, `kotlin/`, and `rust/`, and the tooling that checks repository
rules.
...
- `ts/`: the hand-written TypeScript reference translation, published as
  the `@boring/codec` package, a member of the bun workspace at the root
  `package.json`. `ts/gen/` is the gitignored
  tree generated by the reflaxe TypeScript target; `verify` regenerates it
  first and the behavior and structure guards run against that output.
- `haxe/`: the Haxe capability samples; every file under `haxe/src/`
  ...
  standard library lives under `haxe/src/std/`; the `haxe.*` and `std`
  namespaces are compile-input identities only and never reach target
  output (docs/specs/stdlib/06-std-modules.md).
- `rust/`: the hand-written Rust reference translation, a member of the
  cargo workspace at the root `Cargo.toml`.
- `kotlin/`: the hand-written Kotlin reference translation, compiled by
  `kotlinc` directly with no build tool.
  `kotlin/gen/` is the gitignored tree generated by the reflaxe Kotlin
  target; `verify` regenerates it first and `test:kotlin` runs its checks
  against that output.
...
- `tools/`: the ESLint plugin, the documentation style checker, the
  commit tool, the git hooks, the vector generator, the reflaxe smoke
  check, and the reflaxe targets (`tools/reflaxe/ts/` and
  `tools/reflaxe/kotlin/` with their `ts.hxml` and `kotlin.hxml`
  entries; the shared runtime-package configuration that both read lives
  in `tools/haxe/RuntimeConfig.hx`).
