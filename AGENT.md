...
corpus under `haxe/src/`, the hand-written reference translations under
...
- `haxe/`: the Haxe capability corpus; every file under `haxe/src/`
  demonstrates language capabilities of the translatable subset, and the
  corpus grows as the accepted construct set grows.
...
- `tools/`: the ESLint plugin, the documentation style checker, the
  ...
  check, and the reflaxe TypeScript target (`tools/reflaxe/ts/` with its
  `ts.hxml` entry).
