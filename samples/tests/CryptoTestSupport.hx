...
class CryptoTestSupport {
    public static function data(name:String):Bytes {
        ...
        if (name == "abc")
            return ascii([97, 98, 99]);
        if (name == "digits")
            return ascii([49, 50, 51, 52, 53, 54, 55, 56, 57]);
        var length = 0;
        if (name == "bytes0to255")
            length = 256;
        else if (name == "len1")
            ...
            length = 241;
        else if (name == "len1003")
            length = 1003;
        else if (name == "len1004")
            length = 1004;
        final bytes = Bytes.alloc(length);
        for (i in 0...length)
            bytes.set(i, name == "bytes0to255" ? i & 255 : (i * 31 + 7) & 255);
        return bytes;
    }
    ...
    public static function signed32(raw:Float):Int
        return raw >= 2147483648 ? Std.int(raw - 4294967296) : Std.int(raw);
