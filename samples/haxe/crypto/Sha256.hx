...
class Sha256 {
    ...
    static function processBlock(hash:Array<Int>, input:Bytes):Void {
        ...
        for (i in 16...64)
            words[i] = safeAdd(safeAdd(safeAdd(gamma1(words[i - 2]), words[i - 7]), gamma0(words[i - 15])), words[i - 16]);
        var a = hash[0];
        var b = hash[1];
        var c = hash[2];
        var d = hash[3];
    ...
    static function rotate(value:Int, distance:Int):Int
        return (value >>> distance) | (value << (32 - distance));

    static function choose(x:Int, y:Int, z:Int):Int
        return (x & y) ^ (~x & z);

    static function majority(x:Int, y:Int, z:Int):Int
        return (x & y) ^ (x & z) ^ (y & z);

    static function sigma0(x:Int):Int
        return rotate(x, 2) ^ rotate(x, 13) ^ rotate(x, 22);

    static function sigma1(x:Int):Int
        return rotate(x, 6) ^ rotate(x, 11) ^ rotate(x, 25);

    static function gamma0(x:Int):Int
        return rotate(x, 7) ^ rotate(x, 18) ^ (x >>> 3);

    static function gamma1(x:Int):Int
