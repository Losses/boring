...
class Sha512 {
	...
	public function new() {
		state = [Int64.make(0x6A09E667, 0xF3BCC908), Int64.make(0xBB67AE85, 0x84CAA73B), Int64.make(0x3C6EF372, 0xFE94F82B), Int64.make(0xA54FF53A, 0x5F1D36F1), Int64.make(0x510E527F, 0xADE682D1), Int64.make(0x9B05688C, 0x2B3E6C1F), Int64.make(0x1F83D9AB, 0xFB41BD6B), Int64.make(0x5BE0CD19, 0x137E2179)];
	...
	public function digest():Bytes {
		...
		if (position > 112) {
			for (i in position...BLOCK_LENGTH) finalBlock.set(i, 0);
			processBlock(finalState, finalBlock);
			for (i in 0...BLOCK_LENGTH) finalBlock.set(i, 0);
		} else for (i in position...112) finalBlock.set(i, 0);
		for (i in 112...128) finalBlock.set(i, 0);
		var bitLength = totalLength << 3;
		for (i in 0...8) finalBlock.set(120 + i, Int64.getLow(bitLength >>> (56 - i * 8)));
		processBlock(finalState, finalBlock);
		var output = new BytesBuffer();
		for (i in 0...8) {
			var word = finalState[i];
			output.addByte(Int64.getLow(word >>> 56)); output.addByte(Int64.getLow(word >>> 48));
			output.addByte(Int64.getLow(word >>> 40)); output.addByte(Int64.getLow(word >>> 32));
			output.addByte(Int64.getLow(word >>> 24)); output.addByte(Int64.getLow(word >>> 16));
			output.addByte(Int64.getLow(word >>> 8)); output.addByte(Int64.getLow(word));
		}
	...
	static function processBlock(hash:Array<Int64>, input:Bytes):Void {
		...
		for (i in 0...80) { var t1 = add4(h, sigma1(e), choose(e,f,g), add(k[i],words[i])); var t2 = add(sigma0(a), majority(a,b,c)); h=g; g=f; f=e; e=add(d,t1); d=c; c=b; b=a; a=add(t1,t2); }
		hash[0]=add(hash[0],a); hash[1]=add(hash[1],b); hash[2]=add(hash[2],c); hash[3]=add(hash[3],d); hash[4]=add(hash[4],e); hash[5]=add(hash[5],f); hash[6]=add(hash[6],g); hash[7]=add(hash[7],h);
	}
	static function add(a:Int64,b:Int64):Int64 return a+b;
	static function add4(a:Int64,b:Int64,c:Int64,d:Int64):Int64 return a + b + c + d;
	static function choose(x:Int64,y:Int64,z:Int64):Int64 return (x & y) ^ (~x & z);
	static function majority(x:Int64,y:Int64,z:Int64):Int64 return (x & y) ^ (x & z) ^ (y & z);
	static function rotate(x:Int64,n:Int):Int64 return (x >>> n) | (x << (64 - n));
	static function sigma0(x:Int64):Int64 return rotate(x,28) ^ rotate(x,34) ^ rotate(x,39);
	static function sigma1(x:Int64):Int64 return rotate(x,14) ^ rotate(x,18) ^ rotate(x,41);
	static function gamma0(x:Int64):Int64 return rotate(x,1) ^ rotate(x,8) ^ (x >>> 7);
	static function gamma1(x:Int64):Int64 return rotate(x,19) ^ rotate(x,61) ^ (x >>> 6);
}