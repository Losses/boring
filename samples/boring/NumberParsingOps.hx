...
class NumberParsingOps {
    ...
    private static function trim(value:String):String {
        ...
        while (start < end && isTrim(value.charCodeAt(start)))
            start++;
        while (end > start && isTrim(value.charCodeAt(end - 1)))
            end--;
        return value.substring(start, end);
    }

    private static function isDigit(code:Int):Bool
    ...
    private static function validFloat(value:String):Bool {
        ...
        if (i < length && (value.charCodeAt(i) == 43 || value.charCodeAt(i) == 45))
            i++;
        var digits = 0;
    ...
    private static function validInt(value:String):Bool {
        ...
        if (i < length && (value.charCodeAt(i) == 43 || value.charCodeAt(i) == 45))
            i++;
        var hexadecimal = i + 1 < length && value.charCodeAt(i) == 48 && (value.charCodeAt(i + 1) == 120 || value.charCodeAt(i + 1) == 88);
        if (hexadecimal)
            i += 2;
        var digits = 0;
