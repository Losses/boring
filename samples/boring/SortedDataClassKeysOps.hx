...
class RubySpan {
    ...
    public function new(baseRange:TextRange, text:String, fontFamilies:ReadOnlyArray<String>, kind:RubyKind, enumKinds:ReadOnlyArray<RubyKind>,
            ranges:ReadOnlyArray<TextRange>, ?locale:String) {
        this.baseRange = baseRange;
        this.text = text;
        this.fontFamilies = fontFamilies;
        this.kind = kind;
        this.enumKinds = enumKinds;
        this.ranges = ranges;
...
class SortedDataClassKeysOps {
    public static function read():String {
        ...
        b.put(new TextRange(4, 8), "b");
        b.put(new TextRange(1, 2), "a");
        final m = b.build();
        final q = SortedMap.builder();
        q.put(new QuotePair(2, 4, QuoteType.Close), "close");
        q.put(new QuotePair(2, 4, QuoteType.Open), "open");
        final qm = q.build();
        final r = SortedMap.builder();
        r.put(new RubySpan(new TextRange(3, 4), "z", ["serif"], RubyKind.Other, [RubyKind.Other], [new TextRange(3, 4)]), "nested");
        r.put(new RubySpan(new TextRange(1, 2), "a", ["serif"], RubyKind.Other, [RubyKind.Bopomofo], [new TextRange(1, 2)]), "nested-first");
        final rm = r.build();
        return m.valueAt(0)
            + ","
            + m.valueAt(1)
    ...
    public static function arrayMutations():String {
        ...
        beforeBuilder.put(a, "a");
        beforeBuilder.put(b, "b");
        ...
        afterBuilder.put(c, "c");
        afterBuilder.put(d, "d");
        final after = afterBuilder.build();
        final prefixBuilder:SortedMapBuilder<RubySpan, String> = SortedMap.builder();
        prefixBuilder.put(new RubySpan(new TextRange(1, 2), "x", ["serif"], RubyKind.Other, [RubyKind.Other, RubyKind.Bopomofo], [new TextRange(1, 2)]),
            "long");
    ...
    public static function nullableMutation():String {
        ...
        mapBuilder.put(valueKey, "value");
        mapBuilder.put(nullKey, "null");
