...
class EnumText {
    ...
    static function labeledForm(value:Expr, e:EnumType, pos:Position, selfName:Null<String>, root:EnumType):Expr {
        ...
        for (ctor in e.constructs) {
            ...
            final pattern:Expr = args.length == 0 ? {expr: EConst(CIdent(ctor.name)), pos: pos} : {
    ...
    static function arrayFormWithType(elementType:Type, read:Expr, pos:Position, selfName:Null<String>, root:Null<EnumType>,
            arrayWrittenType:Null<ComplexType>):Expr {
        ...
        final body:Expr = {
            expr: EBlock([
                ...
                {
                    expr: EFor({
                        expr: EBinop(OpIn, index,
                            {expr: EBinop(OpInterval, {expr: EConst(CInt("0")), pos: pos}, {expr: EField(arrayIdent, "length"), pos: pos}), pos: pos}),
                        pos: pos
                    }, {
                        expr: EBlock([
                            {
                                expr: EIf({expr: EBinop(OpGt, index, {expr: EConst(CInt("0")), pos: pos}), pos: pos},
                                    {expr: ECall({expr: EField(out, "add"), pos: pos}, [{expr: EConst(CString(", ")), pos: pos}]), pos: pos}, null),
                                pos: pos
                            },
                            {
