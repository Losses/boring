		...
		switch(fn.expr) {
			...
			case TField(subj, FStatic(c, cf)):
				...
				if(cls.module == "Std" && (fName == "parseFloat" || fName == "parseInt") && args.length == 1) {
					final s = expr(args[0]);
					if(fName == "parseFloat") return "((s) => { let a = 0, z = s.length; const w = function(c) { return c === 32 || (c >= 9 && c <= 13); }; while (a < z && w(s.charCodeAt(a))) { a++; } while (z > a && w(s.charCodeAt(z - 1))) { z--; } let i = a; const d = function() { const st = i; while (i < z && s.charCodeAt(i) >= 48 && s.charCodeAt(i) <= 57) { i++; } return i > st; }; const c0 = s.charCodeAt(i); if (c0 === 43 || c0 === 45) i++; const before = d(); let ok; if (i < z && s.charCodeAt(i) === 46) { i++; const after = d(); ok = before || after; } else { ok = before; } if (ok && i < z) { const e = s.charCodeAt(i); if (e === 101 || e === 69) { i++; const c1 = s.charCodeAt(i); if (c1 === 43 || c1 === 45) i++; ok = d(); } else { ok = false; } } return ok && i === z ? Number.parseFloat(s.substring(a, z)) : Number.NaN; })(" + s + ")";
					return "((s) => { let a = 0, z = s.length; const w = (c) => c === 32 || (c >= 9 && c <= 13); while (a < z && w(s.charCodeAt(a))) a++; while (z > a && w(s.charCodeAt(z - 1))) z--; let i = a; const c0 = s.charCodeAt(i); if (c0 === 43 || c0 === 45) i++; let radix = 10; if (i + 1 < z && s.charCodeAt(i) === 48 && (s.charCodeAt(i + 1) === 120 || s.charCodeAt(i + 1) === 88)) { i += 2; radix = 16; } const st = i; if (radix === 16) { while (i < z) { const c = s.charCodeAt(i); if ((c >= 48 && c <= 57) || (c >= 97 && c <= 102) || (c >= 65 && c <= 70)) i++; else break; } } else { while (i < z && s.charCodeAt(i) >= 48 && s.charCodeAt(i) <= 57) i++; } if (i === st || i !== z) return null; const n = Number.parseInt(s.substring(a, z), radix); return n >= -2147483648 && n <= 2147483647 ? n : null; })(" + s + ")";
				}
