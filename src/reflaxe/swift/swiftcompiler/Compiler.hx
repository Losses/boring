...
class Compiler extends PluginCompiler<Compiler> {
	...
	**/
	...
	public override function generateFilesManually() {
		...
		for(module in modules) {
			...
			final moduleUsesFoundation = moduleUsesFoundationFor(module);
			final content = (moduleUsesFoundation ? "import Foundation\n\n" : "") + GENERATED_HEADER + "\n" + body + "\n";
	...
	**/
	...
	function moduleUsesFoundationFor(module: String): Bool {
		if(!contexts.exists(module)) return false;
		final decl: SwiftDecl = cast contexts.get(module);
		return decl.usesFoundation();
	}
