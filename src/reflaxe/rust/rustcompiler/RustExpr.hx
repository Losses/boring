		...
		switch(fn.expr) {
			...
			case TField(_, FStatic(c, cf)):
				...
				if(cls.module == "Std" && name == "parseInt") {
					return "{ let t = (" + expr(args[0]) + ").trim_matches(|c: char| c.is_ascii_whitespace()); if t.starts_with(\"0x\") || t.starts_with(\"0X\") || t.starts_with(\"+0x\") || t.starts_with(\"+0X\") || t.starts_with(\"-0x\") || t.starts_with(\"-0X\") { let negative = t.starts_with('-'); let d = if negative || t.starts_with('+') { &t[1..] } else { t }; i32::from_str_radix(&d[2..], 16).ok().map(|n| if negative { -n } else { n }) } else { t.parse::<i32>().ok() } }";
				}
