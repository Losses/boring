		...
		switch(fn.expr) {
			...
			case TField(_, FStatic(c, cf)):
				...
				if(cls.module == "Std" && name == "parseInt") {
					return "{ let t = (" + expr(args[0]) + ").trim_matches(|c: char| matches!(c, ' ' | '\\t' | '\\n' | '\\u{0B}' | '\\u{0C}' | '\\r')); let (negative, d) = if t.starts_with('-') { (true, &t[1..]) } else if t.starts_with('+') { (false, &t[1..]) } else { (false, t) }; if d.starts_with("0x") || d.starts_with("0X") { i64::from_str_radix(&d[2..], 16).ok().map(|n| if negative { -n } else { n }).filter(|n| *n >= -2147483648 && *n <= 2147483647).map(|n| n as i32) } else if !negative { t.parse::<i64>().ok().filter(|n| *n <= 2147483647).map(|n| n as i32) } else { t.parse::<i64>().ok().filter(|n| *n >= -2147483648).map(|n| n as i32) } }";
				}
