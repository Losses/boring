...
class GraphemeData {
	...
	**/
	...
	static function readDataFile(fileName: String): String {
		...
		if(!sys.FileSystem.exists(path)) {
			Context.fatalError("pinned Unicode data file is missing: " + path + " (expected next to the compilation root; run from the repository root, or refresh with -D fetch-unicode=<version>)", Context.currentPos());
		}
	...
	**/
	...
	static function loadCache(digest: String): Bool {
		...
		cachedTable = [for(part in lines[1].split(",")) if(StringTools.trim(part).length > 0) Std.parseInt(StringTools.trim(part))];
			...
			for(i in 0...codes.length) {
				if(actual[i] != expected[i]) {
					...
					if(firstFailure == null) {
						firstFailure = "line " + StringTools.trim(rawLine) + ": boundary before U+" + StringTools.hex(codes[i]).toUpperCase() + " expected " + expected[i] + " got " + actual[i];
					}
					break;
				}
			}
